jcdfObject = function() {
    ...
    return {
		...
		//	window.parent.jcdfDialog(window.self.name, href, title, maxHeight, maxWidth, widthRate);
		...
		refreshJcdfDatagrid:function(datagridId, type) {
			...
			if ($("#"+datagridId).length > 0) {
				...
				window.parent.$$.refreshJcdfDatagrid(datagridId, type);
			}
		},
		/**
		 *	刷新数据树，如果采用了JCDF框架的弹窗，就必需采用JCDF的该方法进行树刷新，因为JCDF的弹窗页面
		 *	与列表页面在两个iframe页面中，无法进行js直接调用。
		 *
		 *	@param	treeId	将要属性的treeId的id
		 */
		refreshJcdfTree:function(treeId) {
			//如果树页面就在当前页面，则直接刷新，否则需要跨iframe进行树刷新
			if ($("#"+treeId).length > 0) {
				$('#'+treeId).tree('reload');
			} else {
				window.parent.$$.refreshJcdfTree(treeId);
			}
		},
		/**
		...
		invokeParentMethod:function(fn, args) {
			...
			//	return eval('window.top.invokeParentMethod('+methodStr+')');
				return eval('invokeParentMethod('+methodStr+')');
			}
			return null;
		},
	...
	//		window.top.modalDialog(window.self.name, href, title, maxHeight, maxWidth, widthRate);
			modalDialog(href, title, maxHeight, maxWidth, widthRate);
		},
...
function jcdfDialog(href, title, maxHeight, maxWidth, widthRate, closable) {	
...
//		dataType : "json",
       ...
       dataFilter: function(data,type){
    	   ...
    	   var res = eval('('+data+")");
		   if(res && res.errorCode == 99998){
			   window.top.$.messager.alert('提示消息', res.msg, 'warning', function(){
				   forward2Login();
			   });	
			   return;
		   }
...
function forward2Login(){
	window.location = appId + '/login.html';
}
	...
	function generateButtons(buttons,pageId,authActions, expellActions){
		...
		for(var i=0;i<buttons.length;i++){
			if(buttons[i].type=="B"){
				...
				var iconCls = icon==null || icon=="null" || icon=="" ? "icon-edit" : icon;
				lis+= 
					"<a id='"+buttons[i].id+"' href='#' onclick=\"doAction('"+pageId+"','"+buttons[i].id+"','"+buttons[i].url+"')\" class='easyui-linkbutton' iconCls='"+iconCls+"' plain='true'>"+buttons[i].name+"</a>\n";
			}
		}
