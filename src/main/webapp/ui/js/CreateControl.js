...
function OnExportBegin(OptionObject) 
{
	    ...
	    OptionObject.AbortOpenFile = false;  //导出后不用关联程序打开导出文件，如导出Excel文件之后不用Excel打开
...
function CellClicked(sender){
	$('body').focus();
	grclickFunc(sender);
}
function OnBatchFetchRecord(){

	if(totalRec!=0 && curpage  > Math.ceil(totalRec/grWantRecords) ){return;}	

	grFetchRecords(curpage,grWantRecords);		
	isSearching = false;
    curpage++;
}
function refreshData(total,details){
	var records  = { "recordset":details };	
	totalRec = 	total;				
	ReportViewer.TotalRecord = total;
	ReportViewer.Report.LoadDataFromXML(JSON.stringify(records));
	
}
function getReportViewHTML(ReportURL,RowsPerPage,WantRecords,OnContentCellClick,onFetchRecords)
{
	grRowsPerPage = RowsPerPage;
	grWantRecords = WantRecords;
	grclickFunc = OnContentCellClick;
	grFetchRecords = onFetchRecords;
    var typeid;
    if( _gr_isIE )
        typeid = 'classid="clsid:600CD6D9-EBE1-42cb-B8DF-DFB81977122E" ' + gr_CodeBase;
    else
        typeid = 'type="application/x-grplugin6-displayviewer"';
    var PluginID = "ReportViewer";
    var Width="100%";
    var Height="100%";
    var DataURL = "";
    var AutoRun = false;
    var ExParams =  "<param name=RowsPerPage value="+RowsPerPage+">" +
				    "<param name=BatchGetRecord value=true>" +
				    "<param name=BatchWantRecords value="+WantRecords+">" +
				    "<param name=OnContentCellClick value='CellClicked'>" +
				    "<param name=OnBatchFetchRecord value='OnBatchFetchRecord'>";
    var objecthtml = "";
    objecthtml += ('<object id="' + PluginID + '" ' + typeid);
    objecthtml +=(' width="' + Width + '" height="' + Height + '">');
    objecthtml +=('<param name="ReportURL" value="' + ReportURL + '">');
    objecthtml +=('<param name="DataURL" value="' + DataURL + '">');
    objecthtml +=('<param name="AutoRun" value=' + AutoRun + '>');
    objecthtml +=('<param name="SerialNo" value="' + gr_SerialNo + '">');
    objecthtml +=('<param name="UserName" value="' + gr_UserName + '">');
    objecthtml +=(ExParams);
    objecthtml +=('</object>');
    return objecthtml;
}
