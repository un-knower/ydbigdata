...
@Service(value="CIP_meta_str_hisService")
public class CIP_meta_str_hisServiceImpl implements CIP_meta_str_hisService {
	
	@Autowired
	private CIP_meta_str_hisDao dataDao = null;

	/** 
	 * 修改数据
	 */
	@Override
	public void updateData(CIP_meta_str_hisVO vo, CIPRuntimeOperator operateInf){
		//TODO
		CIP_meta_str_hisPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_meta_str_hisPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(CIP_meta_str_hisVO vo, CIPRuntimeOperator operateInf){
		CIP_meta_str_hisPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_meta_str_hisPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public CIP_meta_str_hisVO getData(Object[] keys ){
		CIP_meta_str_hisPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<CIP_meta_str_hisData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<CIP_meta_str_hisData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
