...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_meta_jobController</p>
 *
 * @since 2015-10-08 09:03:37
 */
@RestController(value="CIP_meta_jobController")
@RequestMapping(value = "/actions/cip_meta_job")
public class CIP_meta_jobController {
	
	@Autowired
	private CIP_meta_jobService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_meta_job_addData";
	/**
	* 新增元数据作业
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_meta_jobVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_meta_job_getData";
	/**
	* 查看元数据作业
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_meta_jobVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_meta_jobVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_meta_job_deleteData";
	/**
	* 删除元数据作业
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_meta_jobVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_meta_job_updateData";
	/**
	* 更新元数据作业
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_meta_jobVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_meta_job_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_meta_jobData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_meta_job_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_meta_jobMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_meta_jobMapper.getExcelTitle(), entities);
				}
			}
