 ...
 */
public class CIP_meta_fields_hisVO {
    ...
    */
	...
	public CIP_meta_fields_hisPO toPO(){
		CIP_meta_fields_hisPO po = new CIP_meta_fields_hisPO();
		
		po.setField_id(field_id);
		po.setStr_id(str_id);
		po.setSys_type(sys_type);
		po.setVersion_date(new Date(DateUtils.parseDate(version_date).getTime()));
		po.setField_memo(field_memo);
		po.setField_name(field_name);
		po.setSearch_flag(search_flag);
		po.setDomain_type(domain_type);
		po.setJava_type(java_type);
		po.setJdbc_type(jdbc_type);
		po.setField_length(field_length);
		po.setFprecision(fprecision);
		po.setField_default(field_default);
		po.setKey_flag(key_flag);
		po.setNull_flag(null_flag);
		po.setField_use_type(field_use_type);
		po.setUi_control(ui_control);
		po.setUi_ctrl_clazz(ui_ctrl_clazz);
