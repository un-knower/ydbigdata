...
@Service(value="CIP_meta_str_logService")
public class CIP_meta_str_logServiceImpl implements CIP_meta_str_logService {
	
	@Autowired
	private CIP_meta_str_logDao dataDao = null;

	/** 
	 * 修改数据
	 */
	@Override
	public void updateData(CIP_meta_str_logVO vo, CIPRuntimeOperator operateInf){
		//TODO
		CIP_meta_str_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_meta_str_logPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(CIP_meta_str_logVO vo, CIPRuntimeOperator operateInf){
		CIP_meta_str_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_meta_str_logPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public CIP_meta_str_logVO getData(Object[] keys ){
		CIP_meta_str_logPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<CIP_meta_str_logData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<CIP_meta_str_logData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
