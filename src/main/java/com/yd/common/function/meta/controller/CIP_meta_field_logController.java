...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_meta_field_logController</p>
 *
 * @since 2015-10-08 09:03:37
 */
@RestController(value="CIP_meta_field_logController")
@RequestMapping(value = "/actions/cip_meta_field_log")
public class CIP_meta_field_logController {
	
	@Autowired
	private CIP_meta_field_logService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_meta_field_log_addData";
	/**
	* 新增数据结构字段变更
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_meta_field_logVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_meta_field_log_getData";
	/**
	* 查看数据结构字段变更
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_meta_field_logVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_meta_field_logVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_meta_field_log_deleteData";
	/**
	* 删除数据结构字段变更
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_meta_field_logVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_meta_field_log_updateData";
	/**
	* 更新数据结构字段变更
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_meta_field_logVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_meta_field_log_searchData";
...
/*			CIPReqCondition[] otherConds = CIPRuntime.authManager.getAuthConditions(C_ACTION_SEARCH_DATA, "authOrg");
			...
			List<CIP_meta_field_logData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_meta_field_log_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_meta_field_logMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_meta_field_logMapper.getExcelTitle(), entities);
				}
			}
