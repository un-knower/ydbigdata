...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_admin_user_settingController</p>
 *
 * @since 2015-07-27 02:40:52
 */
@RestController
@RequestMapping(value = "/actions/admin_user_setting")
public class CIP_admin_user_settingController {
	
	@Autowired
	private CIP_admin_user_settingService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "CIP_admin_user_setting_addData";
	/**
	* 新增用户信息设置
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_admin_user_settingVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "CIP_admin_user_setting_getData";
	/**
	* 查看用户信息设置
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_admin_user_settingVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			CIP_admin_user_settingVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "CIP_admin_user_setting_deleteData";
	/**
	* 删除用户信息设置
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_admin_user_settingVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "CIP_admin_user_setting_updateData";
	/**
	* 更新用户信息设置
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_admin_user_settingVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "CIP_admin_user_setting_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_admin_user_settingData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "CIP_admin_user_setting_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_admin_user_settingMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_admin_user_settingMapper.getExcelTitle(), entities);
				}
			}
