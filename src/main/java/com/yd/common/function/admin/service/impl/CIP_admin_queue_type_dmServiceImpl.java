...
@Service
public class CIP_admin_queue_type_dmServiceImpl implements CIP_admin_queue_type_dmService {
	
	@Autowired
	private CIP_admin_queue_type_dmDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(CIP_admin_queue_type_dmVO vo, CIPRuntimeOperator operateInf){
		//TODO
		CIP_admin_queue_type_dmPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_admin_queue_type_dmPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(CIP_admin_queue_type_dmVO vo, CIPRuntimeOperator operateInf){
		CIP_admin_queue_type_dmPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_admin_queue_type_dmPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public CIP_admin_queue_type_dmVO getData(Object[] keys ){
		CIP_admin_queue_type_dmPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<CIP_admin_queue_type_dmData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<CIP_admin_queue_type_dmData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("cip_admin_queue_type_dm");
			log.setOp_obj_id("");
			log.setOp_type("E");
