...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_admin_questionController</p>
 *
 * @since 2015-12-25 01:45:43
 */
@RestController(value="CIP_admin_questionController")
@RequestMapping(value = "/actions/admin_question")
public class CIP_admin_questionController {
	
	@Autowired
	private CIP_admin_questionService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_admin_question_addData";
	/**
	* 新增系统问题反馈
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_admin_questionVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_admin_question_getData";
	/**
	* 查看系统问题反馈
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_admin_questionVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_admin_questionVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_admin_question_deleteData";
	/**
	* 删除系统问题反馈
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_admin_questionVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_admin_question_updateData";
	/**
	* 更新系统问题反馈
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_admin_questionVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_HANDLE_DATA = "cip_admin_question_handleData";
	/**
	* 处理系统问题反馈
	*/
	@RequestMapping(value = "/handleData")
	public CIPResponseMsg handleData(@RequestBody CIP_admin_questionVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			// lock.setAuthValue("authAtribute", String.valueOf(vo.getAttribute()));
			// xAuth = CIPRuntime.authManager.checkBehavior(C_ACTION_HANDLE_DATA, lock);
			if(xAuth){
				dataService.handleData(vo, CIPRuntime.getOperateSubject());
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			}
			else{
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_admin_question_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			if(conditonStr!=null){
				...
				for( String key : keys ){
					...
					if(value==null || value.trim().equals(""))
						continue;
					if("isAdmin".equals(key)&& !"root".equals(CIPRuntime.getOperateSubject().getSubject_id())){
						if("0".equals(value)){
							req = new CIPReqCondition();
							req.setFieldName("creater");
							req.setLowValue(CIPRuntime.getOperateSubject().getSubject_id());
			...
			List<CIP_admin_questionData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_admin_question_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_admin_questionMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_admin_questionMapper.getExcelTitle(), entities);
				}
			}
