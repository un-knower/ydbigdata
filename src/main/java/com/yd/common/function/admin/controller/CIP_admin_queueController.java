...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_admin_queueController</p>
 *
 * @since 2015-08-18 02:49:22
 */
@RestController
@RequestMapping(value = "/actions/admin_queue")
public class CIP_admin_queueController {
	
	@Autowired
	private CIP_admin_queueService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "CIP_admin_queue_addData";
	/**
	* 新增系统队列信息
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_admin_queueVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "CIP_admin_queue_getData";
	/**
	* 查看系统队列信息
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_admin_queueVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			CIP_admin_queueVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "CIP_admin_queue_deleteData";
	/**
	* 删除系统队列信息
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_admin_queueVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "CIP_admin_queue_updateData";
	/**
	* 更新系统队列信息
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_admin_queueVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "CIP_admin_queue_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_admin_queueData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "CIP_admin_queue_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_admin_queueMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_admin_queueMapper.getExcelTitle(), entities);
				}
			}
