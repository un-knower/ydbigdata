...
import com.yd.common.function.admin.data.mapper.CIP_admin_log_mdmMapper;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_admin_log_mdmController</p>
 *
 * @since 2015-08-26 08:45:49
 */
@RestController
@RequestMapping(value = "/actions/admin_log_mdm")
public class CIP_admin_log_mdmController {
	
	@Autowired
	private CIP_admin_log_mdmService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "CIP_admin_log_mdm_addData";
	/**
	* 新增本地日志-主数据变更
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_admin_log_mdmVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "CIP_admin_log_mdm_getData";
	/**
	* 查看本地日志-主数据变更
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_admin_log_mdmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			CIP_admin_log_mdmVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "CIP_admin_log_mdm_deleteData";
	/**
	* 删除本地日志-主数据变更
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_admin_log_mdmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "CIP_admin_log_mdm_updateData";
	/**
	* 更新本地日志-主数据变更
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_admin_log_mdmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "CIP_admin_log_mdm_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_admin_log_mdmData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "CIP_admin_log_mdm_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_admin_log_mdmMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_admin_log_mdmMapper.getExcelTitle(), entities);
				}
			}
