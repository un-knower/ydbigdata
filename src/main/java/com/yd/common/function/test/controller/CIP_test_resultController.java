...
import com.yd.common.function.test.data.mapper.CIP_test_resultMapper;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_test_resultController</p>
 *
 * @since 2015-10-10 02:52:52
 */
@RestController(value="CIP_test_resultController")
@RequestMapping(value = "/actions/test_result")
public class CIP_test_resultController {
	
	@Autowired
	private CIP_test_resultService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_test_result_addData";
	/**
	* 新增系统测试结果
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_test_resultVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_test_result_getData";
	/**
	* 查看系统测试结果
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_test_resultVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_test_resultVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_test_result_deleteData";
	/**
	* 删除系统测试结果
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_test_resultVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_test_result_updateData";
	/**
	* 更新系统测试结果
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_test_resultVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_test_result_searchData";
...
//				conditions = otherConds;
			...
			List<CIP_test_resultData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_test_result_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_test_resultMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_test_resultMapper.getExcelTitle(), entities);
				}
			}
