...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_test_template_formController</p>
 *
 * @since 2015-11-24 04:16:55
 */
@RestController(value="CIP_test_template_formController")
@RequestMapping(value = "/actions/test_template_form")
public class CIP_test_template_formController {
	
	@Autowired
	private CIP_test_template_formService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_test_template_form_addData";
	/**
	* 新增测试数据模板字段
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_test_template_formVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_test_template_form_getData";
	/**
	* 查看测试数据模板字段
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_test_template_formVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_test_template_formVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_test_template_form_deleteData";
	/**
	* 删除测试数据模板字段
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_test_template_formVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_test_template_form_updateData";
	/**
	* 更新测试数据模板字段
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_test_template_formVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_test_template_form_searchData";
...
//			}
			...
			List<CIP_test_template_formData> vos = dataService.searchData(page,conditions);
...
private static final String C_ACTION_SEARCH_TEMPLATEDATA = "cip_test_template_form_searchTemplateData";
	
	/**
	* 查询指定测试模板的字段信息
	*/
	@RequestMapping(value = "/searchTemplateData")
	public CIPResponseQueryMsg searchData(@RequestBody String templateid) {
		CIPResponseQueryMsg msg = new CIPResponseQueryMsg();
		try {
			String[] template = templateid.split("=");
			List<CIP_test_template_formVO> vos = dataService.searchTemplateData(template[1]);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_test_template_form_exportEntities";
...
//			CIPReqCondition[] otherConds = CIPRuntime.authManager.getAuthConditions(C_ACTION_SEARCH_DATA, "authOrg");
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_test_template_formMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_test_template_formMapper.getExcelTitle(), entities);
				}
			}
