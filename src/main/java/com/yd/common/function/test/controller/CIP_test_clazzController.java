...
import com.yd.common.function.test.data.mapper.CIP_test_clazzMapper;
import com.yd.common.function.test.data.vo.CIP_test_clazzVO;
import com.yd.common.function.test.service.CIP_test_clazzService;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_test_clazzController</p>
 *
 * @since 2015-10-10 02:52:52
 */
@RestController(value="CIP_test_clazzController")
@RequestMapping(value = "/actions/test_clazz")
public class CIP_test_clazzController {
	
	@Autowired
	private CIP_test_clazzService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_test_clazz_addData";
	/**
	* 新增系统测试类
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_test_clazzVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_test_clazz_getData";
	/**
	* 查看系统测试类
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_test_clazzVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_test_clazzVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_test_clazz_deleteData";
	/**
	* 删除系统测试类
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_test_clazzVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_test_clazz_updateData";
	/**
	* 更新系统测试类
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_test_clazzVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_test_clazz_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_test_clazzData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_test_clazz_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_test_clazzMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_test_clazzMapper.getExcelTitle(), entities);
				}
			}
