...
import com.yd.common.function.test.data.mapper.CIP_test_caseMapper;
import com.yd.common.function.test.data.vo.CIP_test_caseVO;
import com.yd.common.function.test.service.CIP_test_caseService;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_test_caseController</p>
 *
 * @since 2015-10-10 02:52:52
 */
@RestController(value="CIP_test_caseController")
@RequestMapping(value = "/actions/test_case")
public class CIP_test_caseController {
	
	@Autowired
	private CIP_test_caseService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_test_case_addData";
	/**
	* 新增系统测试用例
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_test_caseVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_test_case_getData";
	/**
	* 查看系统测试用例
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_test_caseVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_test_caseVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_test_case_deleteData";
	/**
	* 删除系统测试用例
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_test_caseVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_test_case_updateData";
	/**
	* 更新系统测试用例
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_test_caseVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_test_case_searchData";
...
//			}
			...
			List<CIP_test_caseData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_test_case_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_test_caseMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_test_caseMapper.getExcelTitle(), entities);
				}
			}
