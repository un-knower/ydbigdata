...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_test_scen_2_caseController</p>
 *
 * @since 2015-10-10 02:52:52
 */
@RestController(value="CIP_test_scen_2_caseController")
@RequestMapping(value = "/actions/test_scen_2_case")
public class CIP_test_scen_2_caseController {
	
	@Autowired
	private CIP_test_scen_2_caseService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_test_scen_2_case_addData";
	/**
	* 新增测试场景用例关系
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_test_scen_2_caseVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_test_scen_2_case_getData";
	/**
	* 查看测试场景用例关系
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_test_scen_2_caseVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_test_scen_2_caseVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_test_scen_2_case_deleteData";
	/**
	* 删除测试场景用例关系
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_test_scen_2_caseVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_test_scen_2_case_updateData";
	/**
	* 更新测试场景用例关系
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_test_scen_2_caseVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_test_scen_2_case_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_test_scen_2_caseData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_test_scen_2_case_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_test_scen_2_caseMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_test_scen_2_caseMapper.getExcelTitle(), entities);
				}
			}
