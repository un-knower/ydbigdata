...
import com.yd.common.function.test.dao.CIP_test_clazzDao;
...
@Service(value="CIP_test_clazzService")
public class CIP_test_clazzServiceImpl implements CIP_test_clazzService {
	
	@Autowired
	private CIP_test_clazzDao dataDao = null;

	/** 
	 * 修改数据
	 */
	@Override
	public void updateData(CIP_test_clazzVO vo, CIPRuntimeOperator operateInf){
		//TODO
		CIP_test_clazzPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_test_clazzPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(CIP_test_clazzVO vo, CIPRuntimeOperator operateInf){
		CIP_test_clazzPO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_test_clazzPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public CIP_test_clazzVO getData(Object[] keys ){
		CIP_test_clazzPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<CIP_test_clazzData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<CIP_test_clazzData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
