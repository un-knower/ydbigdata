...
@Service(value="CIP_test_caseService")
public class CIP_test_caseServiceImpl implements CIP_test_caseService {
	
	@Autowired
	private CIP_test_caseDao dataDao = null;

	/** 
	 * 修改数据
	 */
	@Override
	public void updateData(CIP_test_caseVO vo, CIPRuntimeOperator operateInf){
		//TODO
		CIP_test_casePO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_test_casePO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(CIP_test_caseVO vo, CIPRuntimeOperator operateInf){
		CIP_test_casePO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_test_casePO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public CIP_test_caseVO getData(Object[] keys ){
		CIP_test_casePO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<CIP_test_caseData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<CIP_test_caseData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
