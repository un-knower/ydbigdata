...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_test_clazz_medController</p>
 *
 * @since 2015-10-10 02:52:52
 */
@RestController(value="CIP_test_clazz_medController")
@RequestMapping(value = "/actions/test_clazz_med")
public class CIP_test_clazz_medController {
	
	@Autowired
	private CIP_test_clazz_medService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_test_clazz_med_addData";
	/**
	* 新增测试类方法
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_test_clazz_medVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_test_clazz_med_getData";
	/**
	* 查看测试类方法
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_test_clazz_medVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
			...
			if(xAuth){
				...
				CIP_test_clazz_medVO vo0 = dataService.getData(keys);
				msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
				msg.msg = CIPErrorCode.CALL_SUCCESS.name;
				msg.data = vo0;
			}
	...
	private static final String C_ACTION_DELETE_DATA = "cip_test_clazz_med_deleteData";
	/**
	* 删除测试类方法
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_test_clazz_medVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_test_clazz_med_updateData";
	/**
	* 更新测试类方法
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_test_clazz_medVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			boolean xAuth = true;
			// CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("authObject");
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_test_clazz_med_searchData";
...
//				conditions = otherConds;
			...
			List<CIP_test_clazz_medData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_SEARCH_ALLDATA = "cip_test_clazz_med_searchAllData";
	/**
	 * 依据sysid查询
	 */
	@RequestMapping(value = "/searchAllData")
	public CIPResponseQueryMsg searchAllData(String sysid) {
		CIPResponseQueryMsg msg = new CIPResponseQueryMsg();
		try {
			
			List<CIP_test_clazz_medPO> vos = dataService.searchAllData(sysid);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_test_clazz_med_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_test_clazz_medMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_test_clazz_medMapper.getExcelTitle(), entities);
				}
			}
