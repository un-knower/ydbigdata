	...
	public static CIPHttpSession getSession(HttpServletRequest request, HttpServletResponse response)  throws Exception {
		...
		if(cookies !=null && cookies.length>0) {
			for(Cookie cookie : cookies) {
				if(cookie.getName().equalsIgnoreCase(CIPRuntimeConfigure.cip_system_id+"_"+CIPRuntimeConstants.CIP_SESSION_ID)) {
		...
		if(cipSession == null) {
			//向客户端回写cookie
			writeCookie(response, CIPRuntimeConfigure.cip_system_id+"_"+CIPRuntimeConstants.CIP_SESSION_ID, sessionId);
