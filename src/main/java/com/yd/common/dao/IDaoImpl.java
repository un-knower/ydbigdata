...
public abstract class IDaoImpl<Data> implements IDao<Data>{
	  ...
	  @LogAnnotation(module="IDao",operateMode="isExist")
		...
		public boolean isExist(Object... keys) throws CIPDaoException {
	    	try {
				...
				int o = jdbcTemplate.queryForObject(getWhereKeysCondition(sql), keys, Integer.class);
				if(o==0)
					return false;
				else 
					return true;
	...
	@Override
	public List<Data> getByCondition(Map<String,Object> conditions) throws CIPDaoException{
		try {
...
//		log.info("sql:"+sql);
	...
	protected String getSql(Object[] args, CIPPageInfo page, boolean isCount){
		...
		//3.分页
		if(page!=null) buffer= new StringBuffer(getSqlPage(buffer,page));
	 ...
	 * @return
	 */
	private StringBuffer getOraPage(StringBuffer querySql, CIPPageInfo page){
		...
		.append(querySql).append(" and rownum<="+(page.getStartRecord()+page.getRows())+") page) where row_number > "+page.getStartRecord());
		return pageQuerySql;
	}
	/**
	 * 拼接分页sql(mysql实现)
	 ...
	 * @return
	 */
	private StringBuffer getMySqlPage(StringBuffer querySql, CIPPageInfo page){
		StringBuffer pageQuerySql = new StringBuffer();
	 ...
	 * @return
	 */
	public StringBuffer getSqlPage(StringBuffer querySql, CIPPageInfo page){
		StringBuffer pageQuerySql = new StringBuffer();
		try {
			if(dbProduct==null){
			   dbProduct = jdbcTemplate.getDataSource().getConnection().getMetaData().getDatabaseProductName();
			}
			if(dbProduct.toLowerCase().contains("mysql")){
			 ...
			 new CIPDaoException(CIPErrorCode.ERROR_DATABASE_TECH_EXCEPTION);
		}		
	...
	protected  String generateWhere(CIPReqCondition[] conditions, List<Object> params){
		StringBuffer whereSql = new StringBuffer().append(" where 1=1 ");
		if(conditions!=null && conditions.length>0) {
			int operator;
			for(int i=0 ; i<conditions.length; i++){
				if(conditions[i].getLowValue() != null ){
					 if(conditions[i].getLowValue().contains("%")){
					   conditions[i].setOperator(8);
					}
				}
