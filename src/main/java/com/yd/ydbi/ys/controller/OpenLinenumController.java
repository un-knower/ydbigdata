...
@RestController
@RequestMapping(value = "/action/openline")
public class OpenLinenumController {
	
	private Logger log = Logger.getLogger(OpenLinenumController.class);
	@Autowired
	YS_carLine yS_carLine;
	
	@RequestMapping(value = "/carline")
	public CustomResponseMsg index(CustomReqParameter parameter) {
		...
		try {
			...
			msg.data=yS_carLine.searchDate(paramsMap);
			msg.errorCode = 0;
			msg.msg = "操作成功";
