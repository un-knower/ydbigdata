 ...
 */
@RestController
@RequestMapping(value = "/action/ydbi_quality_control")
public class Ydbi_quality_controlController {

	private Logger logger = Logger.getLogger(Ydbi_quality_controlController.class);

	@Autowired
	Ydbi_quality_controlService ydbi_quality_controlService;
	...
	public CustomResponseMsg searchData(CustomReqParameter parameter) {
		...
		List<YdbiQualityControl> cvpBoardList = new ArrayList<YdbiQualityControl>();
		cvpBoardList = (List<YdbiQualityControl>) ydbi_quality_controlService.searchData(paramsMap);
